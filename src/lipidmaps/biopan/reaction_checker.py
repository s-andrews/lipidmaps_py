        ...
        try:
            ...
            def names_list(comps):
                ...
                for c in comps:
                    name = (
                        c.get("compound_name")
                        or c.get("compound_lm_id")
                        or c.get("compound_generic_id")
                        or ""
                    )
                    if name:
                        parts.append(name)
            ...
            for reaction in data:
                ...
                filtered_reactants = [
                    c for c in (reactants or []) if c.get("compound_type") == "lm_main"
                ]
                filtered_products = [
                    c for c in (products or []) if c.get("compound_type") == "lm_main"
                ]

...
class LipidSpecies(BaseModel):
    ...
    def parse_name_and_chains(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if notation == LipidNotationType.SUM_COMPOSITION:
            ...
            if mm:
                chains.append(
                    FattyAcidChain(
                        carbons=int(mm.group(1)), double_bonds=int(mm.group(2))
                    )
                )
        elif notation in (
            LipidNotationType.SPECIES_LEVEL,
            LipidNotationType.MOLECULAR_SPECIES,
        ):
    ...
    def get_sum_composition(self) -> str:
        if (
            self.lipid_class
            and self.total_carbons is not None
            and self.total_double_bonds is not None
        ):
...
def create_lipid_species(
    name: str, lm_id: Optional[str] = None, subclass: Optional[str] = None
) -> LipidSpecies:

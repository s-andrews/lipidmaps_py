...
class DataValidator:
    ...
    def _validate_structure(
        self, raw_df: RawDataFrame, report: ValidationReport
    ) -> None:
        ...
        duplicates = [
            name for name, count in Counter(raw_df.fieldnames).items() if count > 1
        ]
        if duplicates:
            report.issues.append(
                ValidationIssue(
        ...
        empty_cols = [
            i
            for i, name in enumerate(raw_df.fieldnames)
    ...
    def _generate_summary(
        ...
        return {
            ...
            "critical_issues": len(
                report.get_issues_by_severity(IssueSeverity.CRITICAL)
            ),
    ...
    def _validate_row_structure(
        ...
        issues = (
            raw_df.metadata.get("row_structure_issues", {})
            if isinstance(raw_df.metadata, dict)
            else {}
        )

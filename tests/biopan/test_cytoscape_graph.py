import json
import pytest
from lipidmaps.biopan.models.cytoscape import CytoscapeGraph, GraphBuilder
...
def test_cytoscape_graph_creation_and_export(graph_builder, tmp_path):
    ...
    graph_builder.graph.add_lipid_node(
        lipid_id="LMFA01010001", name="Palmitic acid", formula="C16H32O2", mass=256.24
    )
    graph_builder.graph.add_reaction_node(
        reaction_id="123", name="Fatty acid synthesis", pathway="Lipid metabolism"
    )
    # Add edge
    graph_builder.graph.add_interaction_edge(
        source="LMFA01010001", target="rxn_123", interaction_type="substrate"
    )
    ...
    assert any(
        node.data.name == "Fatty acid synthesis" for node in graph.elements.nodes
    )

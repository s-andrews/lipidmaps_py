...
class NodeData(BaseModel):
    ...
    type: Optional[str] = Field(None, description="Node type (e.g., 'lipid', 'reaction', 'pathway')")
    
    # Lipid-specific fields
    lm_id: Optional[str] = Field(None, description="LIPID MAPS ID")
    standardized_name: Optional[str] = Field(None, description="RefMet standardized name")
    ...
    significant: Optional[bool] = Field(None, description="Whether change is significant")
    
    # Additional custom attributes
    parent: Optional[str] = Field(None, description="Parent node ID for hierarchical graphs")
...
class CytoscapeNode(BaseModel):
    ...
    position: Optional[NodePosition] = Field(None, description="Node position in 2D space")
...
class EdgeData(BaseModel):
    ...
    label: Optional[str] = Field(None, description="Edge label (e.g., 'substrate', 'product')")
    ...
    # Reaction-specific fields
    stoichiometry: Optional[float] = Field(None, description="Stoichiometric coefficient")
    reversible: Optional[bool] = Field(None, description="Whether reaction is reversible")
...
class GraphElements(BaseModel):
    ...
    nodes: List[CytoscapeNode] = Field(default_factory=list, description="List of nodes")
    edges: List[CytoscapeEdge] = Field(default_factory=list, description="List of edges")
    ...
    def get_edges_for_node(self, node_id: str) -> List[CytoscapeEdge]:
        ...
        return [e for e in self.edges if e.data.source == node_id or e.data.target == node_id]


class GraphMetadata(BaseModel):
    """Metadata about the graph."""
    generated_by: str = Field(default="BioPAN Python", description="Tool that generated the graph")
...
class LayoutOptions(BaseModel):
    ...
    name: Literal['cose', 'circle', 'grid', 'breadthfirst', 'concentric', 'random'] = Field(
        'cose',
        description="Layout algorithm name"
    )
    animate: bool = Field(True, description="Whether to animate the layout")
    animationDuration: int = Field(500, description="Animation duration in milliseconds")
    ...
    nodeRepulsion: Optional[int] = Field(None, description="Node repulsion force (COSE)")
    ...
    directed: Optional[bool] = Field(None, description="Use directed layout (breadthfirst)")
...
class StyleSelector(BaseModel):
    ...
    selector: str = Field(description="CSS selector (e.g., 'node', 'edge', '.highlighted')")
...
class CytoscapeGraph(BaseModel):
    ...
    style: Optional[List[StyleSelector]] = Field(None, description="Visual style definitions")
    ...
    def add_interaction_edge(self, source: str, target: str, 
                            interaction_type: str, **kwargs) -> CytoscapeEdge:
...
class GraphBuilder(BaseModel):
    ...
    graph: CytoscapeGraph = Field(default_factory=lambda: CytoscapeGraph(
        elements=GraphElements()
    ))
    
    def build_lipid_reaction_network(self, 
                                     lipids: List[Dict[str, Any]], 
                                     reactions: List[Dict[str, Any]]) -> CytoscapeGraph:
        ...
        self.graph.metadata = GraphMetadata(
            lipid_count=len([n for n in self.graph.elements.nodes if n.data.type == 'lipid']),
            reaction_count=len([n for n in self.graph.elements.nodes if n.data.type == 'reaction']),
            pathway_count=len(set(n.data.pathway for n in self.graph.elements.nodes 
                                if n.data.pathway))
        )

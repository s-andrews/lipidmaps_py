...
class QuantifiedLipid(BaseModel):
    ...
    reactions: Optional[List[Dict[str, Any]]] = None  # e.g., {"reactant": "PC", "product": "LPC", "type": "class-level"}
    ...
    def zscore(self) -> Dict[str, float]:
        ...
        mean = np.mean(vals)
        std = np.std(vals)
        return {k: (v - mean) / std if std != 0 else 0.0 for k, v in self.values.items()}
        ...
        for group, sample_ids in grouped.items():
            result[group] = [
                QuantifiedLipid(
                    ...
                    values={sid: lipid.values[sid] for sid in sample_ids if sid in lipid.values}
                )

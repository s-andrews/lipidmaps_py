...
class RefMet:
    ...
    def annotate_samples(samples: List[Any]) -> Dict[str, Dict[str, Any]]:
        """
        Query RefMet for a list of samples and return a lookup mapping input_name -> result dict.
        Does NOT mutate the sample objects.
        """
        names = [s.sample_name for s in samples]
        data = {"metabolite_name": "\n".join(names)}
        try:
            logger.info("Sending request to RefMet API")
            response = requests.post(RefMet.MWBaseURL, data=data, verify=False, timeout=20)
        ...
        refmet_lookup: Dict[str, Dict[str, Any]] = {}
        for line in lines[1:]:
            fields = line.split("\t")
            input_name = fields[input_idx] if input_idx is not None and len(fields) > input_idx else None
            standardized = fields[standardized_idx] if standardized_idx is not None and len(fields) > standardized_idx else None
            lm_id = fields[lm_id_idx] if lm_id_idx is not None and len(fields) > lm_id_idx else None
            formula = fields[formula_idx] if formula_idx is not None and len(fields) > formula_idx else None
            exact_mass = fields[exact_mass_idx] if exact_mass_idx is not None and len(fields) > exact_mass_idx else None
            super_class = fields[super_class_idx] if super_class_idx is not None and len(fields) > super_class_idx else None
            main_class = fields[main_class_idx] if main_class_idx is not None and len(fields) > main_class_idx else None
            sub_class = fields[sub_class_idx] if sub_class_idx is not None and len(fields) > sub_class_idx else None
            chebi_id = fields[chebi_id_idx] if chebi_id_idx is not None and len(fields) > chebi_id_idx else None
            kegg_id = fields[kegg_id_idx] if kegg_id_idx is not None and len(fields) > kegg_id_idx else None
            refmet_id = fields[refmet_id_idx] if refmet_id_idx is not None and len(fields) > refmet_id_idx else None
            ...
            logger.info(f"refmet result {result_dict['standardized_name']}")
            if input_name:
                refmet_lookup[input_name] = result_dict

        logger.info(f"Annotated {len(refmet_lookup)} samples via RefMet")
        return refmet_lookup

...
class DataManager(BaseModel):
    ...
    def process_csv(self, csv_path: Union[str, Path]) -> LipidDataset:
        ...
        logger.info(f"Created LipidDataset: {len(samples_meta)} samples, {len(quantified)} lipids")
        return dataset
    ...
    def extract_sample_metadata(self, sample_ids: List[str]) -> List[SampleMetadata]:
        ...
        def extract_group(sample_id: str) -> str:
            ...
            return 'unknown'
        return [SampleMetadata(sample_id=sid, group=extract_group(sid)) for sid in sample_ids]

    def extract_quantified_lipids(self, rows: List[Dict], name_col: str, sample_ids: List[str]) -> List[QuantifiedLipid]:
        ...
        for row_idx, row in enumerate(rows, start=1):
            ...
            for sid in sample_ids:
                ...
                try:
                    ...
                    skipped_values += 1
                    logger.warning(f"Non-numeric value for sample {sid} at row {row_idx}: {raw!r}")
                    continue
            if values:
                quantified.append(QuantifiedLipid(input_name=lipid_name, values=values))
                logger.debug(f"Quantified lipid: {lipid_name}, number of values: {len(values)}")
                logger.debug(f"Skipped rows: {skipped_rows}, skipped values: {skipped_values}")
            else:
                skipped_rows += 1
    ...
    def annotate_lipids_with_refmet(self, quantified: List[Any]) -> None:
        ...
        temp_objs = [SimpleNamespace(sample_name=q.input_name) for q in quantified]
        try:
            refmet_response = RefMet.annotate_samples(temp_objs)
            logger.info(f"Refmet response: {refmet_response}")
            for q in quantified:
                r = refmet_response.get(q.input_name, {})
                q.standardized_name = r.get('standardized_name')
                q.lm_id = r.get('lm_id')
                q.sub_class = r.get('sub_class')
                q.formula = r.get('formula')
                q.mass = r.get('exact_mass')
                q.super_class = r.get('super_class')
                q.main_class = r.get('main_class')
                q.chebi_id = r.get('chebi_id')
                q.kegg_id = r.get('kegg_id')
                q.refmet_id = r.get('refmet_id')
        except Exception:
            logger.exception("RefMet annotation failed; continuing without standardized names")

    def dataset_dict(self) -> Dict[str, Any]:
        """Serialize the dataset to plain dict for JSON output or downstream analysis."""
        if self.dataset is None:
            return {}
        return self.dataset.model_dump() if hasattr(self.dataset, 'model_dump') else self.dataset.dict()
    ...
    def get_group_statistics(self) -> Dict[str, Dict[str, Any]]:
        ...
        for group_name, samples in groups.items():
            ...
            for lipid in self.dataset.lipids:
                ...
                group_values = [lipid.values.get(sid) for sid in sample_ids 
...
if __name__ == '__main__':
    ...
    for group_name, stats in group_stats.items():
        print(f"  {group_name}: {stats['sample_count']} samples, {stats['lipid_coverage']} lipids with data")
